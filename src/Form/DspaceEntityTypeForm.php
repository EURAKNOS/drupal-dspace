...
use Drupal\drupal_dspace\Entity\DspaceEntity;
use Drupal\drupal_dspace\Entity\DspaceEntityType;
use Drupal\drupal_dspace\DspaceEntityInterface;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * Form handler for the Dspace entity type add and edit forms.
 */
class DspaceEntityTypeForm extends EntityForm {
  ...
  /**
   * The Dsprace storage client manager.
   *
  ...
  /**
   * Constructs an DspaceEntityTypeForm object.
   *
  ...
  /**
   * Provides an edit Dspace entity type title callback.
   *
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $dspace_entity_type = $this->getEntity();

    if ($this->operation == 'add') {
      $form['#title'] = $this->t('Add Dspace entity type');
    }
    else {
      $form['#title'] = $this->t('Edit %label Dspace entity type', ['%label' => $dspace_entity_type->label()]);
    }
    ...
    $form['label'] = [
      ...
      '#title' => $this->t('Name'),
      '#maxlength' => 255,
      '#default_value' => $dspace_entity_type->label(),
      '#description' => $this->t('The human-readable name of this Dspace entity type. This name must be unique.'),
      '#required' => TRUE,
    ];

    $form['id'] = [
      '#type' => 'machine_name',
      '#default_value' => $dspace_entity_type->id(),
      '#machine_name' => [
        'exists' => [$this, 'exists'],
      ],
      '#maxlength' => EntityTypeInterface::ID_MAX_LENGTH,
      '#disabled' => !$dspace_entity_type->isNew(),
    ];
    ...
    $form['label_plural'] = [
      ...
      '#maxlength' => 255,
      '#default_value' => $dspace_entity_type->getPluralLabel(),
      '#description' => $this->t('The plural human-readable name of this Dspace entity type.'),
      '#required' => TRUE,
    ];
    ...
    $form['description'] = [
      ...
      '#description' => $this->t("Description of the Dspace entity type."),
      '#default_value' => $dspace_entity_type->getDescription(),
    ];

    $form['read_only'] = [
      '#title' => $this->t('Read only'),
      '#type' => 'checkbox',
      '#default_value' => $dspace_entity_type->isReadOnly(),
      '#description' => $this->t('Whether or not this Dspace entity type is read only.'),
    ];
    ...
    $base_fields = DspaceEntity::defaultBaseFieldDefinitions();
    $fields = $dspace_entity_type->getDerivedEntityType()
      ? $this->entityFieldManager->getFieldDefinitions($dspace_entity_type->getDerivedEntityTypeId(), $dspace_entity_type->getDerivedEntityTypeId())
      : $base_fields;
    // Filter out fields from the annotation entity.
    foreach ($fields as $key => $field) {
      if (strpos($key, DspaceEntityInterface::ANNOTATION_FIELD_PREFIX) === 0 || $key === DspaceEntityInterface::ANNOTATION_FIELD || $field->isComputed()) {
        unset($fields[$key]);
      }
    }
      ...
      foreach ($property_definitions as $property_name => $property_definition) {
        if (!$property_definition->isReadOnly()) {
          ...
          $element = [
            '#title' => $title,
            '#type' => 'textfield',
            '#default_value' => $dspace_entity_type->getFieldMapping($field_name, $property_name),
    ...
    $period[DspaceEntityType::CACHE_DISABLED] = '<' . $this->t('no caching') . '>';
    $period[Cache::PERMANENT] = $this->t('Permanent');
    $persistent_cache_max_age = $form_state->getValue(['caching', 'entity_cache_max_age'], DspaceEntityType::CACHE_DISABLED)
        ?: $dspace_entity_type->getPersistentCacheMaxAge();
    $form['caching']['persistent_cache_max_age'] = [
      '#type' => 'select',
      '#title' => $this->t('Persistent cache maximum age'),
      '#description' => $this->t('The maximum time the Dspace entity can be persistently cached.'),
      '#options' => $period,
    ...
    if ($annotations_enable === NULL) {
      $annotations_enable = $dspace_entity_type->isAnnotatable();
    }
    $form['annotations']['annotations_enable'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Enable annotations on this Dspace entity type'),
      '#description' => $this->t('Annotations allow to enrich dspace entities with locally stored data (such as Drupal fields). This is achieved by linking local entities with dspace entities using an entity reference field (on the local entity). Before enabling this option, you have to make sure an entity reference field (to the Dspace entity type) is available on the local entity type and/or bundle.'),
    ...
    if ($annotations_enable) {
      ...
      ]) ?: $dspace_entity_type->getAnnotationEntityTypeId();
      ...
      ]) ?: $dspace_entity_type->getAnnotationBundleId();
      ...
      ]) ?: $dspace_entity_type->getAnnotationFieldName();
      ...
      $form['annotations']['annotation_settings']['annotation_field_name'] = [
        ...
        '#description' => $this->t('The entity reference field on the annotation entity which references the Dspace entity.'),
      ...
      ], FALSE) ?: $dspace_entity_type->inheritsAnnotationFields();
      $form['annotations']['annotation_settings']['inherits_annotation_fields'] = [
        '#type' => 'checkbox',
        '#title' => $this->t('Inherit fields'),
        '#description' => $this->t('When enabled, fields and their values are automatically transferred from the annotation to the Dspace entity. Inherited fields are regular entity fields and can be used as such.'),
  ...
  public function buildStorageClientSelectForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($storage_clients as $storage_client_id => $definition) {
      $config = $storage_client_id === $dspace_entity_type->getStorageClientId() ? $dspace_entity_type->getStorageClientConfig() : [];
    ...
    if ($storage_client_options) {
      $form['storage']['storage_client_id'] = [
        ...
        '#default_value' => $dspace_entity_type->getStorageClientId(),
        '#required' => TRUE,
        '#ajax' => [
          'callback' => [get_class($this), 'buildAjaxStorageClientConfigForm'],
          'wrapper' => 'dsprace-entities-storage-client-config-form',
          'method' => 'replace',
          'effect' => 'fade',
        ],
      ];
  ...
  public function buildStorageClientConfigForm(array &$form, FormStateInterface $form_state) {
    ...
    $storage_client_id = $form_state->getValue(['storage', 'storage_client_id']) ?: $dspace_entity_type->getStorageClientId();
    if ($storage_client_id) {
      $storage_client = !$dspace_entity_type->isNew() && $storage_client_id === $dspace_entity_type->getStorageClientId()
        ? $dspace_entity_type->getStorageClient()
    ...
    $form['storage']['storage_client_config']['#attributes']['id'] = 'dsprace-entities-storage-client-config-form';
  }
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    $actions['submit']['#value'] = $this->t('Save Dspace entity type');
    return $actions;
  }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    $form_state->setValue('persistent_cache_max_age', (int) $form_state->getValue(['caching', 'persistent_cache_max_age'], DspaceEntityType::CACHE_DISABLED));
  ...
  public function save(array $form, FormStateInterface $form_state) {
    $dspace_entity_type = $this->getEntity();
    $status = $dspace_entity_type->save();

    if ($status) {
      $this->messenger->addMessage($this->t('Saved the %label Dspace entity type.', [
        '%label' => $dspace_entity_type->label(),
      ]));
    }
    else {
      $this->messenger->addMessage($this->t('The %label Dspace entity type was not saved.', [
        '%label' => $dspace_entity_type->label(),
      ]));
    }

    $form_state->setRedirect('entity.dspace_entity_type.collection');
  }
  ...
  public function getAnnotationEntityTypeIdOptions() {
    ...
    foreach ($definitions as $entity_type_id => $definition) {
      if ($entity_type_id !== 'dspace_entity' && $definition instanceof ContentEntityType) {
